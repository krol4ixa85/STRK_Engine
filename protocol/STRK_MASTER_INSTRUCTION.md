...
/mnt/project/
  STRK_MASTER_INSTRUCTION.md   ~62k · поведение, гейты, протокол RUN
...
│   ИТОГО ~94k символов ≈ ~23k токенов                          │
│                                                                │
│ НЕ ЧИТАЕТ:                                                     │
...
LIQ с v5.2: MASTER (62k) + 3 skills (24k) = ~21k токенов
ЭКОНОМИЯ LIQ: ~54k токенов (72%)
...
ВЫЧИСЛЯЕТСЯ ЗА КАЖДЫЙ RUN (Ярус A):
  ...
  next_unlock_size    размер в STRK и в $ по текущей цене
  next_unlock_pct     % от circulating supply
  daily_emission      усреднённая эмиссия STRK/день (30-дневное окно)
  emission_$_daily    та же цифра в $ по текущей цене
...
СВЯЗЬ С MUST #12 (unlock pressure):
  unlock_pressure = next_unlock_$ × CEX_share / median_daily_vol_$
...
ВЫЧИСЛЯЕТСЯ ЗА КАЖДЫЙ RUN (Ярус A):
  total_dex_tvl_$      суммарная TVL STRK-пар на DEX
  depth_2pct_bid_$     объём в $, нужный для ↓2% цены (продажа)
  depth_2pct_ask_$     объём в $, нужный для ↑2% цены (покупка)
  depth_asymmetry      bid / ask — >1 = легче падать, <1 = легче расти
  cex_spot_vol_24h_$   спот-объём CEX 24ч для сравнения
  dex_vs_cex_ratio     dex_vol_24h / cex_vol_24h

СТРОКА В ОТЧЁТЕ:
  LIQUIDITY: DEX TVL $X · depth ±2%: bid $Y / ask $Z ·
...
Шесть строк (все шесть, даже если NOT_CHECKED):
  ...
  1. REGIME     <trending_up|trending_down|ranging|volatile|quiet>
                · adx=X · bb=Y · eff=Z · conf=C · size_mult=M
                (уже вычисляется в MUST #15, здесь только строкой)

  2. CYCLE_BTC  <UP|NEUTRAL|DOWN> vs MA200 · slope · cycle_mult=M
                (BTC против своего MA200; не путать с REGIME для STRK)

  3. PHASE      <B|C|D|E> · одна фраза «почему»
  ...
  4. STRUCTURE  VAL=$X · POC=$Y · VAH=$Z
                ИЛИ STRUCTURE: NOT_CHECKED (метод не считался этот RUN)

  5. POSITION   цена $X · до VAL: −A% · до инвалидации: −B% · R:R = Z
                (если плана нет — «плана нет, R:R не вычисляется»)

  6. DECISION   → см. блок DECISION выше (не дублировать вердикт)
...
Выполняется В КАЖДОМ RUN, ПЕРЕД сбором Яруса A:
  ...
  1. view /mnt/project/STRK_FORWARDTEST_LOG.md
  2. Найти все прогнозы с status=PENDING И verify_after ≤ сегодня
  3. Для каждого:
  ...
  4. Если хотя бы один прогноз переоценён — печать в отчёт:
       "Пост-мортем R__: <PENDING → HIT/MISS>, прогноз "<строка>",
        outcome $X.XXX"
  5. Дополнительно: если MISS-прогнозы имеют паттерн (несколько подряд
...
ЗАЧЕМ ЭТО СДЕЛАНО:
  1. Экономия ~45 000 токенов на не-RUN сессиях
     (простой вопрос не тянет всю методологию в контекст)
  2. Экономия ~15 000 токенов на RUN
     (короткий системный промпт вместо двойника)
  3. Обновление правил = правка одного файла, не системного промпта
  4. Устранение рассинхрона (нельзя иметь две противоречащие версии)
  5. Prompt-injection resistance: SYSTEM_PROMPT остаётся тонкой
...
```
1. этот файл (STRK_MASTER_INSTRUCTION.md)      — кто я и что делаю
2. STRK_REPORT_TEMPLATE.md                      — что печатать (31 блок, v4.1)
3. STRK_REPORT_TEMPLATE.html                    — КАК печатать: полный визуальный
                                                   образец всех 31 блока — не
                                                   переизобретать вёрстку каждый RUN,
                                                   не рендерить частично (см. §7.8)
4. STRK_BLOCK_SKILLS.md                         — как заполнять каждый блок (v4.1)
5. STRK_FORWARDTEST_LOG.md                      — построчная история прогнозов
                                                   (не текстовый счётчик)
6. simple-memory → тег latest                   — ОБЯЗАТЕЛЬНО прочитать несколько
                                                   записей, не одну — их может быть
                                                   несколько с одинаковым тегом от
                                                   разных сессий того же дня.
                                                   Сортировать по createdAt, брать
                                                   самую свежую (см. §7.9)
7. discord → канал 1502225814714978374          — курация алертов, читать САМ
```
...
ШАГ 3.5 ЗАПОЛНИТЬ БЛОК DECISION по контракту полей (правило 5, §0.5).
        Порядок расчёта:
          1. позиция из memory: есть/нет · вход · размер · стоп
          2. опоры n/4 → CORE on/off
          3. DG strategic / tactical
          4. PLAYBOOK: пробой есть/нет
          5. тайминг: где цена относительно VAL/POC/VAH, догон?
          6. множители SIZE (pillar × cycle × tac_DG), результат в % И $
          7. запись всех полей DECISION
...
4. ГРАНИЦЫ КЛАССИФИКАЦИИ КОШЕЛЬКОВ
...
5. НЕЙТРАЛИТЕТ ПО РЕГУЛЯТОРНЫМ ТЕМАМ
...
8. ВСЕГДА ПРЕДЛАГАТЬ РЕШЕНИЕ (правило от 26.07.2026, по прямому
